...
/* The Interconnect Premium. Site logic.
   Constants below mirror methodology.csv; the CSV is the citable source. */

(function () {
"use strict";

/* ---------- constants (see methodology.csv for sources) ---------- */

var C = {
  bytesPerParam: 2,          /* bf16 */
  allreduceFactor: 2,        /* ring all-reduce, 2(N-1)/N approximated as 2 */
  stepTime: 5,               /* seconds; derived from Llama 3.1 405B throughput */
...
var LINKS = [
  ["home",    0.125e9, "HOME FIBRE 1 Gb/s",            "1G FIBRE",   "ANYWHERE"],
  ["wan",     1.25e9,  null,                            "10G WAN",    "CROSS-REGION"],
  ["campus",  12.5e9,  "DATACENTRE ETHERNET 100 Gb/s", "100G ETH",   "ONE CAMPUS"],
  ["ib",      50e9,    "INFINIBAND 400 Gb/s",          "400G IB",    "ONE BUILDING"],
  ["nvlink",  900e9,   "NVLINK 900 GB/s",              "NVLINK",     "ONE RACK"]
];

function bwGradient(p)  { return C.allreduceFactor * C.bytesPerParam * p / C.stepTime; }
function bwLowcomm(p)   { return bwGradient(p) / C.dilocoLocalSteps; }
function bwEvolution()  { return C.evoPopulation * C.evoBytesPerMember / C.stepTime; }

function cheapestLink(bw) {
  for (var i = 0; i < LINKS.length; i++) if (bw <= LINKS[i][1]) return LINKS[i];
  return null;
}

/* unsharded pure-DP parameters at which sync gradient exits a 400 Gb/s port */
var CROSSOVER_B = 50e9 / (C.allreduceFactor * C.bytesPerParam / C.stepTime) / 1e9; /* 62.5 */

/* ---------- formatting ---------- */

var INK = "#16181D", INK2 = "#5A6068", RULE = "#E4E4E0",
    BLUE = "#1244A6", RED = "#B3362A", PAPER = "#F7F7F5";
var MONO = '"IBM Plex Mono", monospace';

function fmtParams(p) {
  var b = p / 1e9;
  if (b >= 1000) return (b / 1000 === 1 ? "1T" : (b / 1000) + "T");
  if (b >= 10) return Math.round(b) + "B";
  return (Math.round(b * 10) / 10) + "B";
}

function fmtBps(v) {
  var u = [[1e12, "TB/s"], [1e9, "GB/s"], [1e6, "MB/s"], [1e3, "kB/s"], [1, "B/s"]];
  for (var i = 0; i < u.length; i++) {
    if (v >= u[i][0]) {
      var n = v / u[i][0];
      return (n >= 100 ? Math.round(n) : n >= 10 ? Math.round(n * 10) / 10 : Math.round(n * 100) / 100) + " " + u[i][1];
    }
  }
  return "0 B/s";
}
...
function px(n) { return n * (window.devicePixelRatio || 1); }

/* ---------- data strip and premium status ---------- */

function renderStrip(doc) {
  var dateEl = el("strip-date");
  if (!dateEl) return;
  var label = "DATA THROUGH", d = C.refAsOf;
  if (doc && doc.meta && doc.meta.last_pull_utc) {
    var pulled = new Date(doc.meta.last_pull_utc);
    d = doc.meta.last_pull_utc.slice(0, 10);
    if (Date.now() - pulled.getTime() > 48 * 3600 * 1000) label = "LAST GOOD";
  } else {
    label = "LAST GOOD";
  }
  dateEl.innerHTML = '<span class="k">' + label + '</span> <span class="v">' + d + "</span>";
}

function renderPremiumStatus(doc) {
  var box = el("premium-status");
  if (!box) return;
  var days = (doc && doc.days) || [];
  var last = days.length ? days[days.length - 1] : null;
  var began = doc && doc.meta ? doc.meta.collection_began : C.refAsOf;
  var html = '<span class="num">COLLECTION BEGAN ' + began + ".</span> ";
  if (last && last.standalone_median != null) {
    var unit = last.methodology_version === "v2" ? "unique machines" : "asks";
    html += "DAY " + days.length + ": standalone median $" +
      last.standalone_median.toFixed(2) + "/GPU-h (" + unit + " n=" + last.standalone_n +
      "), whole 8+ GPU NVLink nodes " + unit + " n=" + last.clustered_n +
      (last.valid ? ", spread " + last.spread_pct + "%."
                  : ": below the stated minimum of 5 per side, day not valid.");
  }
  html += " The series publishes when both sides clear the minimum; construction and trim rule in the methodology file.";
  box.innerHTML = html;
}

function renderKillChips(doc) {
  var chip1 = el("chip-1");
  if (!chip1) return;
  var valid = ((doc && doc.days) || []).filter(function (d) { return d.valid; });
  chip1.textContent = valid.length ? "WATCHING" : "COLLECTING";
}

function loadData() {
  if (!el("strip-date") && !el("premium-status") && !el("chip-1")) return;
  fetch("data/premium.json")
    .then(function (r) { return r.json(); })
    .then(function (doc) { renderStrip(doc); renderPremiumStatus(doc); renderKillChips(doc); })
    .catch(function () { renderStrip(null); renderPremiumStatus(null); renderKillChips(null); });
}

/* ---------- shared uPlot furniture ---------- */
...
function baseAxis(overrides) {
  var a = {
    ...
    ticks: { show: false },
    gap: 8,
    filter: function (u2, splits) { return splits; }
  };
  for (var k in overrides) a[k] = overrides[k];
  return a;
}

function logSplits(vals) { return function () { return vals; }; }

/* ---------- Exhibit A: the Bandwidth Wall ---------- */

var XA_MIN = 1e9, XA_MAX = 1e12, YA_MIN = 1e2, YA_MAX = 2e12;

function exhibitAData(n) {
  var xs = [], g = [], l = [], e = [];
  for (var i = 0; i <= n; i++) {
    var p = Math.pow(10, 9 + 3 * i / n);
    xs.push(p); g.push(bwGradient(p)); l.push(bwLowcomm(p)); e.push(bwEvolution());
  }
  return [xs, g, l, e];
}

var BANDS = [
  [50e9, 900e9, 0.40, "NVLINK 900 GB/s", "900 GB/s NVLINK"],
  [12.5e9, 50e9, 0.30, "INFINIBAND 400 Gb/s", "400 Gb/s IB"],
  [0.125e9, 12.5e9, 0.22, "DATACENTRE ETHERNET 100 Gb/s", "100 Gb/s ETH"],
  [YA_MIN, 0.125e9, 0.15, "HOME FIBRE 1 Gb/s", "1 Gb/s FIBRE"]
];

function drawBands(u) {
  var ctx = u.ctx;
  ctx.save();
  var left = u.bbox.left, w = u.bbox.width;
  BANDS.forEach(function (b) {
    var y1 = u.valToPos(b[1], "y", true), y0 = u.valToPos(b[0], "y", true);
    ctx.globalAlpha = b[2];
    ctx.fillStyle = RULE;
    ctx.fillRect(left, y1, w, y0 - y1);
  });
  ctx.restore();
}

function drawBandLabels(u, sparse) {
  var ctx = u.ctx;
  ctx.save();
  ctx.font = px(12) + "px " + MONO.replace(/"/g, "");
  ctx.fillStyle = INK2;
  ctx.textAlign = "left";
  ctx.textBaseline = "top";
  BANDS.forEach(function (b, i) {
    if (sparse && i !== 0 && i !== 3) return;
    var y1 = u.valToPos(b[1], "y", true);
    ctx.fillText(sparse ? b[4] : b[3], u.bbox.left + px(10), y1 + px(6));
  });
  ctx.restore();
}

function drawLineLabels(u, labels) {
  var ctx = u.ctx;
  ctx.save();
  ctx.font = "500 " + px(12) + "px " + MONO.replace(/"/g, "");
  ctx.textAlign = "right";
  ctx.textBaseline = "bottom";
  labels.forEach(function (L) {
    var y = u.valToPos(L.y, "y", true);
    ctx.fillStyle = L.color;
    ctx.fillText(L.text, u.bbox.left + u.bbox.width - px(6), y - px(5));
  });
  ctx.restore();
}

function buildExhibitA(target, opts) {
  var interactive = opts.interactive;
  var height = opts.height || 460;
  var data = exhibitAData(140);
  var readout = opts.readout ? el(opts.readout) : null;

  function updateReadout(p) {
    if (!readout) return;
    var rows = [
      ["GRADIENT", bwGradient(p), "g"],
      ["LOW-COMM", bwLowcomm(p), "l"],
      ["EVOLUTIONARY", bwEvolution(), "e"]
    ].map(function (r) {
      var link = cheapestLink(r[1]);
      return '<span class="' + r[2] + '">' + r[0] + " " + fmtBps(r[1]) + "</span>" +
        ' <span class="dim">needs ' + (link ? link[3] : "MORE THAN NVLINK") + "</span>";
    });
    readout.innerHTML = '<span class="dim">AT ' + fmtParams(p) + ":</span> " + rows.join('<span class="dim"> / </span> ');
  }
  ...
  var u = new uPlot({
    ...
    legend: { show: false },
    cursor: interactive
      ? { y: false, points: { show: false }, drag: { x: false, y: false } }
      : { show: false },
    scales: {
      x: { time: false, distr: 3, log: 10, range: [XA_MIN, XA_MAX] },
      y: { distr: 3, log: 10, range: [YA_MIN, YA_MAX] }
    },
    series: [
      {},
      { stroke: RED, width: 2, points: { show: false } },
      { stroke: BLUE, width: 2, dash: [6, 5], points: { show: false } },
      { stroke: BLUE, width: 2, points: { show: false } }
    ],
    axes: [
      baseAxis({
        splits: logSplits([1e9, 3e9, 1e10, 3e10, 1e11, 3e11, 1e12]),
        values: function (u2, s) { return s.map(function (v) { return v == null ? "" : fmtParams(v); }); },
        size: 34
      }),
      baseAxis({
        grid: { show: true, stroke: RULE, width: 1 },
        splits: logSplits([1e3, 1e6, 1e9, 1e12]),
        values: function () { return ["1 kB/s", "1 MB/s", "1 GB/s", "1 TB/s"]; },
        size: 68
      })
    ],
    hooks: {
      drawClear: [drawBands],
      draw: [
        function (u2) { drawBandLabels(u2, height < 400 || u2.width < 560); },
        function (u2) {
          var narrow = u2.width < 560;
          drawLineLabels(u2, [
            { y: bwGradient(XA_MAX), color: RED, text: narrow ? "SYNC" : "SYNC GRADIENT" },
            { y: bwLowcomm(XA_MAX), color: BLUE, text: narrow ? "LOW-COMM" : "LOW-COMM, 500 LOCAL STEPS" },
            { y: bwEvolution() * 1.15, color: BLUE, text: narrow ? "EVOLUTION" : "EVOLUTIONARY" }
          ]);
        }
      ],
      setCursor: interactive ? [function (u2) {
        if (u2.cursor.left != null && u2.cursor.left >= 0) {
          updateReadout(u2.posToVal(u2.cursor.left, "x"));
        }
      }] : []
    }
  }, data, target);
...
/* ---------- the Panel: where can this run ---------- */

function initPanel() {
  var root = el("panel");
  if (!root) return;
  var buttons = root.querySelectorAll(".methods button");
  var slider = el("panel-size");
  var sizeOut = el("panel-size-readout");
  var verdict = el("panel-verdict");
  var detail = el("panel-verdict-detail");
  var method = "gradient";

  function bwFor(p) {
    if (method === "gradient") return bwGradient(p);
    if (method === "lowcomm") return bwLowcomm(p);
    return bwEvolution();
  }

  function update() {
    var p = Math.pow(10, 9 + parseFloat(slider.value));
    sizeOut.textContent = fmtParams(p);
    var billions = Math.round(p / 1e8) / 10;
    slider.setAttribute("aria-valuetext",
      billions >= 1000 ? "1 trillion parameters" : billions + " billion parameters");
    var bw = bwFor(p);
    var link = cheapestLink(bw);
    verdict.textContent = link ? link[4] : "NOWHERE AT THIS STEP TIME";
    detail.textContent = "REQUIRED " + fmtBps(bw) + " PER WORKER. CHEAPEST SUFFICIENT LINK: " +
      (link ? link[3] : "NONE LISTED") + ".";
  }
...
var GEMINI = {
  x: 2023.92,
  label: "GEMINI 1.0 / MULTI-SITE",
  short: "GEMINI MULTI-SITE",
  detail: "SIZE NOT DISCLOSED",
  link: "private inter-datacentre fibre"
};
var REFS = [
  { x: 2020.37, p: 175e9, label: "GPT-3 175B", short: "GPT-3 175B", dx: 8, dy: 14 },
  { x: 2022.29, p: 540e9, label: "PALM 540B", short: "PALM 540B", dx: 8, dy: 14 }
];
...
function buildExhibitC(target) {
  ...
  function col(pred, val) {
    return xs.map(function (x) {
      var hit = null;
      RUNS.concat(REFS).forEach(function (r) { if (r.x === x && pred(r)) hit = r; });
      if (hit) return val(hit);
      return null;
    });
  }
  ...
  var data = [
    xs,
    refLine,
    col(function (r) { return r.kind === "commodity"; }, function (r) { return r.p; }),
    col(function (r) { return !r.kind; }, function (r) { return r.p; })
  ];
  ...
  var u = new uPlot({
    ...
    series: [
      {},
      { stroke: INK2, width: 1, dash: [4, 4], spanGaps: true, points: { show: false } },
      { stroke: BLUE, width: 0, paths: function () { return null; },
        points: { show: true, size: 9, width: 0, fill: BLUE } },
      { stroke: INK2, width: 0, paths: function () { return null; },
        points: { show: true, size: 6, width: 0, fill: INK2 } }
    ],
    axes: [
      baseAxis({
        splits: logSplits([2020, 2021, 2022, 2023, 2024, 2025, 2026]),
        values: function (u2, s) { return s.map(String); },
        size: 34
      }),
      baseAxis({
        grid: { show: true, stroke: RULE, width: 1 },
        splits: logSplits([1e9, 1e10, 1e11, 1e12]),
        values: function () { return ["1B", "10B", "100B", "1T"]; },
        size: 48
      })
    ],
    hooks: {
      draw: [function (u2) {
        ...
        /* Gemini is evidence of private multi-site training, but its parameter
           count was not disclosed. Keep it on a non-quantitative top rail. */
        var gx = u2.valToPos(GEMINI.x, "x", true);
        var gy = u2.bbox.top + px(9);
        ctx.beginPath();
        ctx.arc(gx, gy, px(4.5), 0, Math.PI * 2);
        ctx.fillStyle = PAPER;
        ctx.fill();
        ...
        ctx.textBaseline = "top";
        ctx.fillText(narrow ? GEMINI.short : GEMINI.label,
          narrow ? gx : gx + px(9), gy + px(7));
        ctx.fillText(GEMINI.detail,
          narrow ? gx : gx + px(9), gy + px(21));
      ...
      setCursor: [function (u2) {
        if (!tip) return;
        var cl = u2.cursor.left, ct = u2.cursor.top;
        if (cl == null || cl < 0) { tip.style.display = "none"; return; }
        var best = null, bestD = 24;
        RUNS.concat(REFS).forEach(function (r) {
          var dx2 = u2.valToPos(r.x, "x") - cl, dy2 = u2.valToPos(r.p, "y") - ct;
          var d = Math.sqrt(dx2 * dx2 + dy2 * dy2);
          if (d < bestD) { best = r; bestD = d; }
        });
        var geminiY = 9; /* cursor coords are plot-area relative; rail sits 9px below bbox top */
        var geminiDx = u2.valToPos(GEMINI.x, "x") - cl;
        var geminiD = Math.sqrt(geminiDx * geminiDx + (geminiY - ct) * (geminiY - ct));
        if (geminiD < bestD) { best = GEMINI; bestD = geminiD; }
        ...
        tip.style.top = best === GEMINI ? Math.max(offY + 18, 0) + "px"
          : Math.max(offY + u2.valToPos(best.p, "y") - 14, 0) + "px";
      }]
    }
  }, data, target);
...
var SCENARIOS = {
  holds: [
    ["Scale-up fabric", "ACCRUES", "Every step still moves twice the model per worker; the fabric is the product."],
    ["Optics and networking attach", "ACCRUES", "Bandwidth per shipped FLOP keeps rising and attach rates follow."],
    ["HBM intensity", "ACCRUES", "Adjacent exposure: sold inside the same systems, though the bottleneck it clears is local memory, not fabric."],
    ["Standalone and prior-generation fleets", "COMPRESSES", "In the unsharded five-second case, capacity behind weaker links cannot hold the step."],
    ["Useful-life and depreciation assumptions", "ACCRUES", "Short schedules keep matching physical turnover; no earnings surprise."],
    ["System versus merchant silicon", "ACCRUES", "Integrated rack-scale systems capture margin merchant parts cannot."]
  ],
  compresses: [
    ["Scale-up fabric", "COMPRESSES", "Hundreds of local steps between syncs; the link stops earning its price."],
    ["Optics and networking attach", "COMPRESSES", "Attach rates track the bandwidth requirement down."],
    ["HBM intensity", "RE-RATES", "Per-chip memory need survives; it was never fabric exposure."],
    ["Standalone and prior-generation fleets", "ACCRUES", "Isolated and older capacity re-enters the training pool."],
    ["Useful-life and depreciation assumptions", "RE-RATES", "Longer standalone earning lives move stated depreciation and earnings."],
    ["System versus merchant silicon", "RE-RATES", "Fungible compute shifts the mix toward merchant parts."]
  ]
};

function initScenario() {
  var body = el("scenario-body");
  if (!body) return;
  var buttons = document.querySelectorAll(".scenario-controls button");

  function render(state) {
    body.innerHTML = SCENARIOS[state].map(function (r) {
      var cls = r[1] === "ACCRUES" ? "accrues" : r[1] === "COMPRESSES" ? "compresses" : "rerates";
      return "<tr><td class=\"exposure\">" + r[0] + "</td><td class=\"reading " + cls + "\">" + r[1] +
        "</td><td class=\"rationale\">" + r[2] + "</td></tr>";
    }).join("");
  }
...
function initHeadline() {
  var text = (Math.round(CROSSOVER_B * 10) / 10) + "B";
  var h = el("headline-figure");
  if (h) h.textContent = text;
  var s = el("strip-headline");
  if (s) s.textContent = text;
}

/* ---------- Note: sidenotes collapse to footnotes on narrow screens ---------- */

function initSidenotes() {
  var host = el("footnotes");
  if (!host) return;
  var notes = Array.prototype.slice.call(document.querySelectorAll(".sidenote"));
  var anchors = notes.map(function (n) {
    var a = document.createComment("sn");
    n.parentNode.insertBefore(a, n);
    return a;
  });
  var list = host.querySelector("ol");
  ...
  function layout() {
    if (mq.matches) {
      ...
      notes.forEach(function (n, i) {
        anchors[i].parentNode.insertBefore(n, anchors[i].nextSibling);
      });
    } else {
...
function boot() {
  ...
  var charts = [];
  var aEl = el("exhibit-a");
  if (aEl) charts.push([aEl, function () { return buildExhibitA(aEl, { interactive: true, readout: "a-readout" }); }]);
  var aMini = el("exhibit-a-mini");
  if (aMini) charts.push([aMini, function () { return buildExhibitA(aMini, { interactive: false, height: 320 }); }]);
  var cEl = el("exhibit-c");
  if (cEl) charts.push([cEl, function () { return buildExhibitC(cEl); }]);

  var built = charts.map(function (c) { return { host: c[0], plot: c[1]() }; });
  ...
  window.addEventListener("resize", function () {
    ...
    t = setTimeout(function () {
      built.forEach(function (b) {
        b.plot.setSize({ width: b.host.clientWidth, height: b.plot.height });
      });
    }, 120);
  });
  ...
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(function () {
      built.forEach(function (b) { b.plot.redraw(); });
    });
  }
}
...
/* exposed for the OG and print renderers in scripts/ */
window.IP = { buildExhibitA: buildExhibitA, buildExhibitC: buildExhibitC };
